...
export const VersionDiff = ({ prev, curr }) => {
    return (
        <div className="border border-zinc-200 bg-white">
            <div className="grid grid-cols-12 border-b-2 border-bahri-blue bg-bahri-blue-50 py-2 px-4 label-caps text-bahri-blue">
            ...
            {FIELDS.map(([key, label]) => {
                ...
                return (
                    <div key={key} className="grid grid-cols-12 border-b border-zinc-100 py-2.5 px-4 text-sm">
                        ...
                        <div className={`col-span-4 font-mono ${changed ? "bg-red-100 text-red-900 px-1" : "text-zinc-600"}`}>{a || "—"}</div>
                        <div className={`col-span-5 font-mono ${changed ? "bg-green-100 text-green-900 px-1" : "text-zinc-900"}`}>{b || "—"}</div>
                    </div>
                );
            })}
        </div>
